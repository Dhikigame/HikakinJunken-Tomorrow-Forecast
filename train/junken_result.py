...
def junken_result_nthweek(month, nth_week, result_num, test_data, result_pre_num):
    # print(sazae_result)
    result_pre = junken_num_string(result_pre_num)
    machine_learning_result = junken_machine_learning(result_pre)
    result = junken_num_string(int(result_num))

    winning_losing_result = winning_losing(result, machine_learning_result)

    print("【" + str(month) + "月 第" + str(nth_week) + "週】")
    print("テストデータ：" + str(test_data) + ", ヒカキンさんの手予測：" + str(result_pre))
    print("機械学習の手：", machine_learning_result)
    print("ヒカキンさんの手：", result)
    print("勝敗：", winning_losing_result)
    print()
